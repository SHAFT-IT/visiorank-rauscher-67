...
function fetchCMD(param, settings){
	...
	} else if( settings.mode == 'popular' ) { // No En voir plus For this Feature
		// Popular Mode
    	cmdURL = 'https://api.instagram.com/v1/media/popular?callback=?';

	} else if( settings.mode == 'location' ) { // No En voir plus For this Feature
		// Location Mode
    	cmdURL = 'https://api.instagram.com/v1/locations/' + settings.locationID + '/media/recent/?callback=?';

	} else if( settings.mode == 'multiuser' ){ // No En voir plus For this Feature
		// New - Multi User Mode
...
/* Instagram Location En voir plus
---------------------------------*/
    ...
    if( data.meta.code == 200 ){
        ...
        if( photos.length > 0 ){
			...
			jQuery('.instagram-photo').each(function(index){
				...
				// Clear Any Existing En voir plus Buttons
				jQuery("#seachInstagramLoadMoreContainer").remove();

				// En voir plus Logic
				if( index == photoCount && instagramBrowserNextMax != "Empty" ){
					// En voir plus Button
					jQuery('<div id="seachInstagramLoadMoreContainer"><a class="seachInstagramLoadMore btn btn-inverse">En voir plus</a></div>').appendTo(ibObj);
				}
