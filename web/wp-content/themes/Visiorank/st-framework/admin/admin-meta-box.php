...
include(ST_ADMIN_PATH.'settings-plugins/mtb-gallery.php');
...
function st_add_post_metabox(){
    ...
    if(function_exists('st_page_builder')){
        add_meta_box(__('Page Builder','magazon'), __('Page Builder','magazon'), 'st_page_builder', $box['post_type'] , $box['context'], $box['priority'],$box['settings']);
...
function st_save_meta_box_data($post_id) {
    ...
    // verify nonce
    if (!isset($_POST['st_meta_box_nonce']) || !wp_verify_nonce($_POST['st_meta_box_nonce'], basename(__FILE__))) {
        return $post_id;
    }
