...
function st_open_lightbox(html) {
    var lightboxHTML = '<div id="stlb-overlay"></div>' + '<div id="stlb-lightbox"><div class="stlb-inner">' + html + '</div></div>';
...
jQuery(document).ready(function() {
    ...
    jQuery('.STpanel-box-upload .bp-upload-button').live('click', function() {
        ...
        tb_show(STpanel_options.seletc_image, 'media-upload.php?type=image&tab=library&st_upload=1&st_file_type=all&TB_iframe=true&post_id=0');
        return false;
        */
    ...
    function re_name_ui_meta(g) {
        ...
        jQuery('li', g).each(function(i) {
            ...
            jQuery('.ui-hook', li).attr('name', pre_name + '[' + i + '][hook]');
        });
    }
    ...
    jQuery('.st-ui .st-ui-more').live('click', function() {
        ...
        jQuery('.ui-autoid', temp).val(uniq);
        // set auto ID
        html = temp.html();
