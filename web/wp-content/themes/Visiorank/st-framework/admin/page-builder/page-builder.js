...
function stpb_lightbox_postpbion(){
     ...
     jQuery('#stpblb-lightbox-wrap').css({'display' : 'block'});
...
jQuery(document).ready(function(){
  ...
  var pd_item_width = ['1_1', '3_4','2_3','1_2','1_3','1_4'];
    ...
    // for select layout 
    jQuery('.stpb_pd_w .layout .stpb-layout-item input').live('change',function(){
        var p = jQuery(this).parents('.layout');
        var gp = jQuery(this).parents('.stpb_pd_w');
    ...
    jQuery('.pb-box-upload .pb-upload-button').live('click',function() {
      ...
      // if  input type not is ID
        ...
        tb_show(STpanel_options.seletc_image, 'media-upload.php?type=image&post_id='+post_ID+'&tab=library&st_upload=1&st_file_type=all&TB_iframe=true&post_id=0');
        return false;
    });
    ...
    jQuery('.pb-box-upload .remove_image').live('click',function(){
      ...
      jQuery('.stpb_pd_w .thumbnail .tt').live('change',function(){
            ...
            if(jQuery(this).val()=='video'){
                jQuery('.thumbnail_images',gp).hide(200,function(){
                     jQuery('.thumbnail_video',gp).show();
                });
               
            }else if(jQuery(this).val()=='image'){
                 jQuery('.thumbnail_video',gp).hide();
                 jQuery('.thumbnail_images',gp).hide();
            }else{
                 jQuery('.thumbnail_video',gp).hide(200,function(){
    ...
    //  for page carousel
    jQuery('.stpb_pd_w  .carousel_ibutton').iButton({
        change: function (t){
                var  p = t.parents('.stpb_pd_w');
                 if(t.attr('checked')){
                        jQuery('.carousel_cate',p).show();
                 }else{
                      jQuery('.carousel_cate',p).hide();
                 }
            }
